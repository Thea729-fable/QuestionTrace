---
name: question-trace
description: Generate an evidence-backed, personalized interview prediction question bank from a user's uploaded job description and resume. Use when a user asks for 面试押题、面经调研、面试题库、岗位面试准备、JD 与简历匹配、模拟面试，or wants fresh role-specific interview experiences researched through Museon CLI, including complete reading of selected posts and all relevant question images, with traceable sources and Markdown output.
---

# QuestionTrace

Turn a JD, a resume, and fresh role-specific interview evidence into a traceable preparation-priority question bank for any role with accessible public interview evidence. Use Museon as the live social-media monitoring and research layer: retrieve Xiaohongshu-native posts, deep-read original content, analyze question images, and supplement Nowcoder through public web research. Build a run-specific real interview evidence base before predicting questions; do not substitute generic model recall or a prewritten role bank for retrieval. Treat every prediction as a preparation recommendation, never a guarantee of what an interviewer will ask.
...
Protect personal data. Process inputs locally where the host supports it. Never include credentials in files or output. Never publish or upload a real resume without explicit approval. Do not invent missing experience, metrics, employers, dates, skills, or education.

## Load references by stage

Read these files before the corresponding stage:

- Read [references/jd-resume-mapping.md](references/jd-resume-mapping.md) before analyzing inputs or generating search terms.
...
- Read [references/bailian-optional.md](references/bailian-optional.md) only when Bailian CLI is available or the user explicitly asks for Bailian verification.
...
Use **live research mode** by default. Analyze the current JD and resume, then retrieve fresh evidence for the inferred role.

Use **evidence replay mode** only when the user supplies an existing evidence set, Museon is unavailable after recovery, or a demo explicitly requests replay. Label replay output with the evidence collection date. Never describe replay evidence as a current live search.
...
### 1. Parse the JD and resume

Extract the canonical role, role variants, company if present, business domain, seniority, responsibilities, must-have skills, preferred skills, collaboration expectations, metrics, and likely interview stages.
...
5. inspect every image that may contain questions or process details, batching media only when the tool imposes a limit;
6. transcribe visible questions faithfully and mark uncertain characters;
7. record body completeness, total media count, inspected media count, unreadable media count, and reason;
8. retain the source URL beside the extracted evidence.

Do not count a source as deeply read when its body is truncated and the remainder cannot be opened. Do not claim an inaccessible or unreadable image was analyzed. Exclude incomplete sources from frequency claims unless the specific evidence used is independently visible and the limitation is explicit.
...
Run `scripts/audit_evidence.py <ledger.json>` when Python is available. Fix all blocking errors before using the ledger for scoring. If Python is unavailable, apply the same checks manually and say so in the method note.
...
- Do not require Bailian authorization for the core workflow.
